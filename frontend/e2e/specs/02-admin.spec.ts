...
test.describe('Администратор — редактирование профиля владельца (US-10)', () => {
  ...
  test('обновляет описание и часовой пояс владельца', async ({ page }) => {
    const admin = new AdminPage(page);
    await admin.goto();

    await expect(admin.getOwnerDescriptionInput()).toBeVisible({ timeout: 10000 });
    await expect(admin.getOwnerTimeZoneInput()).toBeVisible();

    await admin.getOwnerDescriptionInput().fill('Обновлённое описание для теста');
    await admin.getOwnerTimeZoneInput().fill('Asia/Yekaterinburg');
    await admin.getSaveOwnerButton().click();
